...
def netbird_status():
    ...
    try:
        result = subprocess.run(["netbird", "status", "--output", "json"], capture_output=True, text=True)
...
def syncthing_status():
    ...
    try:
        ...
        result = subprocess.run(["syncthing", "cli", "config", "devices"], capture_output=True, text=True)
        if result.returncode == 0:
            return json.loads(result.stdout)
...
def firefox_search(query: str, profile: str = "standard"):
    ...
    try:
        ...
        if not profiles: return f"Error: Profile {profile} not found."
        db_path = os.path.join(FIREFOX_PATH, profiles[0], "places.sqlite")
        if not os.path.exists(db_path): return f"Error: Places database not found for {profile}."
        temp_db = f"/tmp/firefox_{profile}_search.sqlite"
        shutil.copy2(db_path, temp_db)
        conn = sqlite3.connect(temp_db); cursor = conn.cursor()
        sql = "SELECT title, url FROM moz_places WHERE (title LIKE ? OR url LIKE ?) ORDER BY last_visit_date DESC LIMIT 10"
        cursor.execute(sql, (f"%{query}%", f"%{query}%"))
        results = cursor.fetchall(); conn.close(); os.remove(temp_db)
...
def check_ai_stack_health(target_host: str = "nixos-nvme", fix: bool = False):
    ...
    try:
        ...
        proc = subprocess.run(eval_cmd, capture_output=True, text=True, cwd=DEFAULT_FLAKE_PATH)
        
        if proc.returncode != 0:
            return {"error": f"Failed to evaluate containers for {target_host}: {proc.stderr}"}
        ...
        for name, data in containers.items():
            ...
            if target_host == "nixos-nvme":
                ...
                if status != "active" and fix:
                    ...
                    status = subprocess.run([SYSTEMCTL, "is-active", unit], capture_output=True, text=True).stdout.strip()
                    results["alerts"].append(f"Restarted container {name}. New status: {status}")
            
            results["containers"][name] = {"status": status, "image": data.get("image", "nixpkgs")}
    ...
    try:
        ...
        for f in files:
            with open(os.path.join(workflow_dir, f), 'r') as file:
                ...
                skills.append({"name": f.replace(".md", ""), "description": desc, "path": os.path.join(workflow_dir, f)})
        return skills
    except Exception as e: return {"error": str(e)}
...
def run_just_recipe(recipe: str, args: list = []):
    ...
    try:
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, cwd=DEFAULT_FLAKE_PATH)
...
def update_todo(task: str, status: str = "done"):
    ...
    try:
        ...
        if not os.path.exists(todo_path):
            with open(todo_path, "w") as f: f.write("# Project Tasks\n\n")
        with open(todo_path, "r") as f: content = f.read()
        mark = "[x]" if status == "done" else "[ ]"
        new_line = f"- {mark} {task}\n"
        if task not in content:
            with open(todo_path, "a") as f: f.write(new_line)
        else:
            old_mark = "[ ]" if status == "done" else "[x]"
            content = content.replace(f"- {old_mark} {task}", new_line)
            with open(todo_path, "w") as f: f.write(content)
        return f"Task updated: {task}"
    except Exception as e: return {"error": str(e)}

@mcp.tool()
def distill_knowledge(title: str, summary: str, context: str, status: str = "active", tags: list = []):
    """
...
def analyze_logs(unit: str = None, machine: str = None, lines: int = 50):
    ...
    """
    try:
        cmd = ["python3", os.path.join(DEFAULT_FLAKE_PATH, "scripts/ai-logs.py"), "--json", "-n", str(lines)]
        if unit: cmd.extend(["-u", unit])
        if machine: cmd.extend(["-m", machine])
        ...
        summary = {}
        for l in logs:
            u = l.get("unit", "unknown")
            if u not in summary: summary[u] = []
            summary[u].append(l.get("message", ""))
    ...
    try:
            ...
            try:
                if engine_type == "ollama":
                    resp = requests.post(f"{base_url}/api/embeddings", 
                                        json={"model": "nomic-embed-text", "prompt": query}, timeout=3)
                else: # llama-cpp
                    resp = requests.post(f"{base_url}/embedding", 
                                        json={"content": query}, timeout=3)
        ...
        for doc in index:
            ...
            if query_embedding and doc.get("embedding"):
                ...
                if query.lower() in doc["title"].lower() or query.lower() in doc["excerpt"].lower():
...
def reindex_vault():
    ...
    """
    try:
        cmd = ["python3", os.path.join(DEFAULT_FLAKE_PATH, "scripts/workspace-indexer.py")]
        result = subprocess.run(cmd, capture_output=True, text=True)
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            if not os.path.exists(GOOGLE_CREDS_PATH):
                raise Exception(f"Google credentials not found at {GOOGLE_CREDS_PATH}. Please download credentials.json from Google Cloud Console.")
            flow = InstalledAppFlow.from_client_secrets_file(GOOGLE_CREDS_PATH, GOOGLE_SCOPES)
        ...
        if os.path.exists(sink_path):
            with open(sink_path, "r") as f:
                ...
                if health_status == "Degraded":
                    subprocess.Popen(["python3", os.path.join(DEFAULT_FLAKE_PATH, "scripts/ai-logs.py"), "--sink"])
        else:
    ...
    try:
        ...
        if target_host == "local":
            if shutil.which("nvidia-smi"):
                res = subprocess.run(["nvidia-smi", "--query-gpu=name,utilization.gpu,memory.used,temperature.gpu", "--format=csv,noheader,nounits"], capture_output=True, text=True)
                if res.returncode == 0:
                    parts = res.stdout.strip().split(", ")
                    profile["gpu"] = {"name": parts[0], "load_pct": parts[1], "mem_used_mb": parts[2], "temp_c": parts[3]}
            
            # Local Jetson check
            teg = shutil.which("tegrastats")
            if teg:
                res = subprocess.run([teg, "--interval", "100", "--count", "1"], capture_output=True, text=True)
...
@mcp.tool()
def search_drive_docs(query: str):
    """Search Google Drive for documents matching a query."""
    try:
        creds = _get_google_creds()
        service = build('drive', 'v3', credentials=creds)
        q = f"name contains '{query}' and (mimeType = 'application/vnd.google-apps.document' or mimeType = 'application/pdf')"
        results = service.files().list(q=q, spaces='drive', fields='files(id, name, mimeType)').execute()
        return results.get('files', [])
    except Exception as e: return {"error": str(e)}

@mcp.tool()
def read_drive_doc(file_id: str):
    """Read the content of a Google Drive document (as text)."""
    try:
        creds = _get_google_creds()
        service = build('drive', 'v3', credentials=creds)
        ...
        return "Error: Unsupported mimeType for direct reading. Use a Google Doc."
    except Exception as e: return str(e)
...
def analyze_nix_closure(attr_path: str):
    ...
    try:
        ...
        res = subprocess.run(cmd, capture_output=True, text=True, cwd=DEFAULT_FLAKE_PATH)
        if res.returncode != 0: return {"error": res.stderr}
...
def get_workspace_state():
    ...
    try:
        if not os.path.exists(STATE_FILE): return "No state found."
...
def check_binary_cache(pkg_name: str):
    ...
    try:
        ...
        cmd = ["nix", "path-info", "--eval", "--json", pkg_name]
        # This is tricky because path-info needs a store path. 
        # A better way is to try to substitute it.
        cmd = ["nix-store", "--realise", "--dry-run", f"/nix/store/...-{pkg_name}"] # Not quite right
        # Let's use a simpler check: try to find it in the public cache via nix search or similar
        res = subprocess.run(["nix", "search", "nixpkgs", pkg_name, "--json"], capture_output=True, text=True)
        return json.loads(res.stdout)
    except Exception as e: return {"error": str(e)}

@mcp.tool()
def manage_skill_progress(skill_name: str, step_index: int = None, status: str = "view"):
    """
    ...
    try:
        ...
        if skill_name not in progress:
            progress[skill_name] = {"completed_steps": [], "last_updated": datetime.now().isoformat()}
