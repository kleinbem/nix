...
def get_machine_status(machine):
    ...
    try:
        result = subprocess.run(
            ["machinectl", "status", machine], capture_output=True, text=True
        )
...
def main():
    ...
    parser.add_argument(
        "-n", "--lines", type=int, default=30, help="Number of log lines to fetch"
    )
    parser.add_argument(
        "-u", "--unit", type=str, help="Specific systemd unit to filter by"
    )
    parser.add_argument(
        "-m", "--machine", type=str, help="Filter by NixOS container/machine name"
    )
    parser.add_argument("--json", action="store_true", help="Output in raw JSON format")
    parser.add_argument(
        "--sink", action="store_true", help="Update the persistent health sink"
    )
        ...
        try:
            ...
            with open(sink_path, "w") as f:
                summary = {
                    ...
                    "recent_units": list(set([log.get("unit") for log in logs]))
    ...
    if args.json:
        ...
        if not logs:
            print(
                f"No recent errors found{' in ' + args.machine if args.machine else ''}. System looks healthy. ✅"
            )
            return
