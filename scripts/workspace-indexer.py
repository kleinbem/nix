...
def get_ollama_url():
    ...
    try:
        inv_path = (
            "/home/martin/Develop/github.com/kleinbem/nix/nix-config/inventory.nix"
        )
        if os.path.exists(inv_path):
            ...
            if result.returncode == 0:
                ...
                if (
                    orin_ip
                    and f"http://{orin_ip}:11434/api/embeddings" not in endpoints
                ):
...
def get_embedding(text):
    try:
        response = requests.post(
            OLLAMA_URL, json={"model": MODEL, "prompt": text}, timeout=5
        )
