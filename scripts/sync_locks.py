...
def main():
    ...
    if dirty_repos:
        print(f"{RED}❌ Cannot perform sync. The following submodules have other uncommitted changes:{RESET}")
    ...
    print(f"🟢 All submodules are clean of non-lockfile changes. Proceeding...\n")
    
    # 2. Update submodules in topological order
    for sub in TOPOLOGICAL_ORDER:
        sub_path = os.path.join(root_dir, sub)
        if not os.path.exists(sub_path) or not os.path.exists(os.path.join(sub_path, "flake.nix")):
            continue
        ...
        if os.path.exists(lock_path):
            try:
                ...
                print(f"  {YELLOW}Warning: Failed to parse flake.lock for overrides in {sub}: {e}{RESET}")
        ...
        success, status = run_cmd(["git", "status", "--porcelain", "flake.lock"], cwd=sub_path, capture=True)
        if success and status.strip():
            ...
            if success:
                print(f"  ✅ Committed new state at {BOLD}{get_git_head(sub_path)}{RESET}")
            else:
                print(f"  {RED}❌ Failed to commit lockfile update in {sub}{RESET}")
                sys.exit(1)
        else:
            print(f"  🟢 {GREEN}Already in sync at {BOLD}{get_git_head(sub_path)}{RESET}")
    ...
    success, status = run_cmd(["git", "status", "--porcelain", "flake.lock"], cwd=root_dir, capture=True)
    if success and status.strip():
    ...
    print(f"To save the updated state in the root repository, run:")
