...
if vulnix --whitelist .agent/audit/whitelist.toml . >>"$REPORT_FILE" 2>&1; then
  ...
  echo "⚠️  Vulnerabilities detected! Check report."
fi
echo '```' >>"$REPORT_FILE"
...
check_option() {
  ...
  if [[ $ACTUAL == "$expected" ]]; then
    ...
    echo "❌ FAIL (Expected: $expected, Got: $ACTUAL)" >>"$REPORT_FILE"
    echo "⚠️  Compliance Fail: $option"
  fi
}
...
if grep -rE "$LEAK_REGEX" . \
  ...
  echo "❌ POTENTIAL LEAK FOUND" >>"$REPORT_FILE"
  echo "🚨 ALERT: Potential sensitive data found! Check report."
else
  echo "✅ No sensitive patterns found." >>"$REPORT_FILE"
