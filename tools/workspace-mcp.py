...
def jj_status():
    ...
    try:
        result = subprocess.run(
            ["jj", "--no-pager", "status"], capture_output=True, text=True, cwd=DEFAULT_FLAKE_PATH
        )
...
def jj_diff(revision: str = "@"):
    ...
    try:
        result = subprocess.run(
            ["jj", "--no-pager", "diff", "-r", revision, "--git"], capture_output=True, text=True, cwd=DEFAULT_FLAKE_PATH
        )
        return result.stdout if result.returncode == 0 else f"Error: {result.stderr}"
    except Exception as e:
        return {"error": str(e)}
...
def jj_commit(message: str):
    ...
    try:
        result = subprocess.run(
            ["jj", "--no-pager", "describe", "-m", message], capture_output=True, text=True, cwd=DEFAULT_FLAKE_PATH
        )
        return result.stdout if result.returncode == 0 else f"Error: {result.stderr}"
    except Exception as e:
        return {"error": str(e)}
...
def jj_log(limit: int = 10):
    ...
    try:
        ...
        result = subprocess.run(
            ...
            capture_output=True, text=True, cwd=DEFAULT_FLAKE_PATH
        )
        if result.returncode != 0:
